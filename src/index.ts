...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    ...
    switch (name) {
      case 'create_draft': {
        const { blog_id, title, body, tags, categories } = args as any;
      ...
      case 'update_last_draft': {
        ...
        const { title, body, tags, categories } = args as any;

        const entry = await mtClient.updateEntry(
          lastEntry.blogId,
          lastEntry.entryId,
      ...
      case 'update_draft': {
        const { blog_id, entry_id, title, body, tags, categories } = args as any;
      ...
      case 'get_draft': {
        const { blog_id, entry_id } = args as any;
      ...
      case 'list_recent_drafts': {
        const { blog_id, limit } = args as any;
